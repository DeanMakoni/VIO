...
//------------------------------------------------------------------------------
Eigen::MatrixXd CoralPNPModel::EvaluateCost(
  ...
  for (const auto &feature : *features) {
    ...
    Eigen::Vector3d point_world = feature->GetTriangulatedPoint();
    Eigen::Vector2d point_uv =
        Eigen::Vector2d(feature->GetSequentialKeyPoint().pt.x,
                        feature->GetSequentialKeyPoint().pt.y);

    //    LOG(INFO) << "Point uv is " << point_uv.transpose() << " and original
    //    is "
    //              << feature->GetValue().transpose();
    //    LOG(INFO)<<" Point projected is "<<point_world.transpose();
    ...
    //    LOG(INFO) << "Point projected is " << ue << " ," << ve;
    //    double err = sqrt((u - ue) * (u - ue) + (v - ve) * (v - ve));

    point_values(feature_no) =
        sqrt((u - ue) * (u - ue) + (v - ve) * (v - ve)) + e;
    feature_no++;
  }
  //  LOG(INFO) << "Point values is \n" << point_values.transpose();

  return point_values;
}
//------------------------------------------------------------------------------
void CoralPNPModel::UpdateModel(
    const utilities::image::FeatureSPtrVectorSptr &features) {
  ...
  for (const auto &feature : *features) {
    ...
    points3d.emplace_back(feature->GetTriangulatedPoint()(0),
                          feature->GetTriangulatedPoint()(1),
                          feature->GetTriangulatedPoint()(2));
