#include "aru/core/coral/extractors/coral_feature_extractor.h"
...
CurvatureExtractor::ExtractAndMatchFeatures(const cv::Mat &image_1,
  ...
  cv::cvtColor(image_1_resized, image_1_grey, cv::COLOR_BGR2GRAY);
  cv::cvtColor(image_2_resized, image_2_grey, cv::COLOR_BGR2GRAY);
  ...
  for (const auto &match : matches) {
    ...
    float u = point_1(0);
    float v = point_1(1);
    // homogenous = homogenous / homogenous(2);
    int pad = 5;
    if (u > pad && u < w - pad && v > pad && v < h - pad) {

      bool good = true;

      int x_cell = u / cell_size;
      int y_cell = v / cell_size;
      ...
      if (good) {
        grid[y_cell * grid_width + x_cell].push_back(
            cv::Point2f((float)u, (float)v));
        features::FeatureSPtr curr_feature =
            boost::make_shared<features::CoralFeatureCurvature>(point_1,
                                                                point_2);
