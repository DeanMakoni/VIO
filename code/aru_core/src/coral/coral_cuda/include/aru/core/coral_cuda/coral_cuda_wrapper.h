...
public:
  ...
  coral::optimiser::EnergyMinimisationResult
  EnergyMinimisation(const Eigen::SparseMatrix<double> neighbourhood_index,
  ...
  void
  FindNearestNeighbours(const utilities::image::FeatureSPtrVectorSptr features,
...
void CoralCudaWrapper<ModelType>::FindNearestNeighbours(
    const utilities::image::FeatureSPtrVectorSptr features,
    cv::Mat &neighbour_index, cv::Mat &inverse_neighbour_index) {
...
CoralCudaWrapper<ModelType>::EnergyMinimisation(
    Eigen::SparseMatrix<double> neighbourhood_index,
    const utilities::image::FeatureSPtrVectorSptr features,
    coral::models::ModelVectorSPtr models) {
  ...
  CudaOptimiser cuda_optimiser(model_costs_cv, neighbour_index_cv,
  ...
  for (int curr_loop = 0; curr_loop < params_.num_loops; ++curr_loop) {
    ...
    this->ReduceModels(models);
    params_.num_labels = this->NumLabels();
    cuda_optimiser.UpdateNumLabels(params_.num_labels);

    if (params_.update_models) {
      this->UpdateModels(features, models);
      Eigen::MatrixXd curr_temp_costs =
          this->EvaluateModelCost(features, models);
      Eigen::MatrixXf curr_model_costs =
          curr_temp_costs.transpose().cast<float>();
      cv::Mat curr_model_costs_cv = Eigen2Cv(curr_model_costs);
      cuda_optimiser.UpdateModelCosts(curr_model_costs_cv);
    }
  }
...
CoralCudaWrapper<ModelType>::EnergyMinimisation(
  ...
  // this->ReduceModels();
  // LOG(INFO)<<"NUmber of labels init is "<<params_.num_labels;
  // params_.num_labels = this->NumLabels();
  // LOG(INFO)<<"NUmber of labels fini is "<<params_.num_labels;
  // cuda_optimiser.UpdateNumLabels(params_.num_labels);
...
CoralCudaWrapper<ModelType>::EnergyMinimisation(
  ...
  // Initialise CUDA Optimiser
  CudaOptimiser cuda_optimiser(model_costs_cv, neighbour_index,
                               inverse_neighbour_index, params_);
