...
#include "matcher.h"
  ...
  public:
    ...
    // feature matches, and simply want to compute visual odometry from them, without using the internal matching functions.
    ...
    float getGain(std::vector<int32_t> inliers_) {
      return matcher->getGain(inliers_);
    }
    ...
    friend std::ostream &operator<<(std::ostream &os, VisualOdometry &viso) {
      ...
      os << p.val[0][0] << " " << p.val[0][1] << " " << p.val[0][2] << " "
         << p.val[0][3] << " ";
      os << p.val[1][0] << " " << p.val[1][1] << " " << p.val[1][2] << " "
         << p.val[1][3] << " ";
      os << p.val[2][0] << " " << p.val[2][1] << " " << p.val[2][2] << " "
         << p.val[2][3];
      return os;
    }
  ...
  protected:
    ...
    estimateMotion(std::vector<Matcher::p_match> p_matched) = 0;
