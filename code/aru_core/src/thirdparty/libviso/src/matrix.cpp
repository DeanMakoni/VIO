...
Boston, MA 02110-1301, USA
                                                    ...
                                                    Edited by Kevin Judd to
remove compiler warnings (kjudd@robots.ox.ac.uk, 2020)
                                                */
...
void Matrix::getData(FLOAT *val_, int32_t i1, int32_t j1, int32_t i2,
                     int32_t j2) {
  if (i2 == -1)
    i2 = m - 1;
  if (j2 == -1)
...
void Matrix::setVal(FLOAT s, int32_t i1, int32_t j1, int32_t i2, int32_t j2) {
  ...
  if (j2 == -1)
    j2 = n - 1;
  if (i2 < i1 || j2 < j1) {
    cerr << "ERROR in setVal: Indices must be ordered (i1<=i2, j1<=j2)."
         << endl;
    exit(0);
  }
  for (int32_t i = i1; i <= i2; i++)
...
Matrix Matrix::diag(const Matrix &M) {
  ...
  cout << "ERROR: Trying to create diagonal matrix from vector of size (" << M.m
       << "x" << M.n << ")" << endl;
  exit(0);
}
...
Matrix Matrix::inv(const Matrix &M) {
  if (M.m != M.n) {
    cerr << "ERROR: Trying to invert matrix of size (" << M.m << "x" << M.n
         << ")" << endl;
    exit(0);
  }
  Matrix A(M);
...
bool Matrix::inv() {
  if (m != n) {
    cerr << "ERROR: Trying to invert matrix of size (" << m << "x" << n << ")"
         << endl;
    exit(0);
  }
  Matrix A(*this);
...
FLOAT Matrix::det() {
  ...
  if (m != n) {
    cerr << "ERROR: Trying to compute determinant of a matrix of size (" << m
         << "x" << n << ")" << endl;
    exit(0);
  }
...
bool Matrix::lu(int32_t *idx, FLOAT &d) {
  // (void)eps; // Avoid unused parameter warning

  if (m != n) {
    cerr << "ERROR: Trying to LU decompose a matrix of size (" << m << "x" << n
         << ")" << endl;
    exit(0);
  }
  ...
  FLOAT *vv = (FLOAT *)malloc(
      n * sizeof(FLOAT)); // vv stores the implicit scaling of each row.
  d = 1.0;
  for (i = 0; i < n;
       i++) { // Loop over rows to get the implicit scaling information.
    big = 0.0;
    for (j = 0; j < n; j++)
...
// Given a matrix M/A[1..m][1..n], this routine computes its singular value
// decomposition, M/A = U·W·V T. Thematrix U replaces a on output. The diagonal
// matrix of singular values W is output as a vector w[1..n]. Thematrix V (not
...
void Matrix::svd(Matrix &U2, Matrix &W, Matrix &V) {
  ...
  for (i = IMIN(m, n) - 1; i >= 0;
       i--) { // Accumulation of left-hand transformations.
    l = i + 1;
    g = w[i];
    for (j = l; j < n; j++)
      U.val[i][j] = 0.0;
  ...
  for (k = n - 1; k >= 0; k--) { // Diagonalization of the bidiagonal form: Loop
