...
Matcher::Matcher(){
  ...
  param=parameters();
  //  param.match_radius=2000;
  //  param.match_disp_tolerance=2000;
  //  param.match_binsize=100;
...
void Matcher::bucketFeatures(int32_t max_features,float bucket_width,float bucket_height) {
  ...
  for (int32_t i=0; i<bucket_cols*bucket_rows; i++) {
    ...
    std::shuffle(std::begin(buckets[i]), std::end(buckets[i]), g);
...
/* kjudd split stereo and termporal match radii */
inline void Matcher::findMatch (int32_t* m1,const int32_t &i1,int32_t* m2,const int32_t &step_size,vector<int32_t> *k2,
                               ...
                               int32_t& min_ind,int32_t stage,bool flow,bool use_prior,int radius,double u_,double v_) {
...
void Matcher::matching (int32_t *m1p,int32_t *m2p,int32_t *m1c,int32_t *m2c,
  ...
  if (method==0) {
    ...
    for (i1c=0; i1c<n1c; i1c++) {
      ...
      findMatch(m1c,i1c,m1p,step_size,k1p,u_bin_num,v_bin_num,stat_bin,i1p, 0,true,use_prior, param.match_radius);
      findMatch(m1p,i1p,m1c,step_size,k1c,u_bin_num,v_bin_num,stat_bin,i1c2,1,true,use_prior, param.match_radius);
    ...
    for (i1c=0; i1c<n1c; i1c++) {
      ...
      // match left/right
      findMatch(m1c,i1c,m2c,step_size,k2c,u_bin_num,v_bin_num,stat_bin,i2c, 0,false,use_prior, param.stereo_radius);
      findMatch(m2c,i2c,m1c,step_size,k1c,u_bin_num,v_bin_num,stat_bin,i1c2,1,false,use_prior, param.stereo_radius);
    ...
    for (i1p=0; i1p<n1p; i1p++) {
      ...
      // match in circle
      findMatch(m1p,i1p,m2p,step_size,k2p,u_bin_num,v_bin_num,stat_bin,i2p, 0,false,use_prior, param.stereo_radius);
      ...
      if (Tr_delta) {
        ...
        findMatch(m2p,i2p,m2c,step_size,k2c,u_bin_num,v_bin_num,stat_bin,i2c, 1,true ,use_prior, param.match_radius,u2c_,v2c_);
      } else {
        findMatch(m2p,i2p,m2c,step_size,k2c,u_bin_num,v_bin_num,stat_bin,i2c, 1,true ,use_prior, param.match_radius);
      }
      findMatch(m2c,i2c,m1c,step_size,k1c,u_bin_num,v_bin_num,stat_bin,i1c, 2,false,use_prior, param.stereo_radius);
      if (Tr_delta)
        findMatch(m1c,i1c,m1p,step_size,k1p,u_bin_num,v_bin_num,stat_bin,i1p2,3,true ,use_prior, param.match_radius,u1p,v1p);
      else{
        findMatch(m1c,i1c,m1p,step_size,k1p,u_bin_num,v_bin_num,stat_bin,i1p2,3,true ,use_prior, param.match_radius);
      }
