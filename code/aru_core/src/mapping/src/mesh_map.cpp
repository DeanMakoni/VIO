...
namespace mesh_map {
//------------------------------------------------------------------------------
MeshMap::MeshMap(std::string mesh_map_settings_file, std::string vocab_file)
    : mesh_map_settings_file_(std::move(mesh_map_settings_file)), vocab_file_(std::move(vocab_file)) {
  ...
  vo_ = boost::make_shared<aru::core::vo::VO>(vocab_file_, extractor_params_,
...
void MeshMap::DrawCurrentTsdf() {
  ...
  BlockIndexList mesh_indices_updated;
  mesh_layer_->getAllUpdatedMeshes(&mesh_indices_updated);
  BlockIndexList mesh_indices_all;
  mesh_layer_->getAllAllocatedMeshes(&mesh_indices_all);
  std::vector<Eigen::Vector3d> voxel_points_;
  std::vector<Eigen::Vector3d> color_points_;
