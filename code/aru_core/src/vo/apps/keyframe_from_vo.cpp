...
DEFINE_string(IMAGE_LEFT_MONO,
              "/home/paulamayo/data/husky_data/log/farm_1_left.monolithic",
              "path to image left monolithic file");
DEFINE_string(IMAGE_RIGHT_MONO,
              "/home/paulamayo/data/husky_data/log/farm_1_right.monolithic",
...
int main(int argc, char *argv[]) {
  ...
  auto *Keyframe = new vo::KeyframeSystem(
      FLAGS_VO_MONO, FLAGS_IMAGE_LEFT_MONO, FLAGS_IMAGE_RIGHT_MONO,
      FLAGS_KEYFRAME_LEFT_MONO, FLAGS_KEYFRAME_RIGHT_MONO);

  auto *viewer = new aru::core::utilities::viewer::VOViewer(
      640, 480, Keyframe->PoseChain());

  auto *viewer_thread =
      new std::thread(&utilities::viewer::VOViewer::Run, viewer);

  auto *vo_thread = new std::thread(&vo::KeyframeSystem::Run, Keyframe);
  // Pangolin viewer needs to be exited and a button pressed in cv Keyframe
  // window for program to terminate Cleaner option and future to-do might be to
  // check if threads are finished runnning
